...
(
 ...
 , rows         BIGINT
 , rows_plan    BIGINT
 , rows_mb      NUMERIC (19, 2)
 , rows_plan_mb NUMERIC (19, 2)
 , mem_mb       NUMERIC (19, 2)
 , mem_plan_mb  NUMERIC (19, 2)
 , read_mb      NUMERIC (19, 2)
 , write_mb     NUMERIC (19, 2)
 , spill_mb     NUMERIC (19, 2)    
 , net_mb       NUMERIC (19, 2)
 , net_cnt      BIGINT
 , run_sec      NUMERIC (9, 1)
...
BEGIN
   ...
   _sql := 'SELECT
      q.query_id::BIGINT                                                          AS query_id
    , e.workers::VARCHAR (16)                                                     AS wrkrs
    , a.rows_actual::BIGINT                                                       AS rows
    , a.rows_planned::BIGINT                                                      AS rows_plan
    , ROUND( a.row_size_actual_bytes / 1024.000^2, 2 )::NUMERIC (19, 2)           AS rows_mb
    , ROUND( a.row_size_planned_bytes / 1024.000^2, 2 )::NUMERIC (19, 2)          AS rows_plan_mb
    , ROUND( a.memory_actual_bytes::bigint    / 1024.000^2, 2 )::NUMERIC (19, 2)  AS mem_mb
    , ROUND( a.memory_planned_bytes::bigint   / 1024.000^2, 2 )::NUMERIC (19, 2)  AS mem_plan_mb
    , ROUND( a.io_read_bytes::bigint          / 1024.000^2, 2 )::NUMERIC (19, 2)  AS read_mb
    , ROUND( a.io_write_bytes::bigint         / 1024.000^2, 2 )::NUMERIC (19, 2)  AS write_mb
    , ROUND( a.io_spill_space_bytes::bigint   / 1024.000^2, 2 )::NUMERIC (19, 2)  AS spill_mb
    , ROUND( a.io_network_bytes::bigint       / 1024.000^2, 2 )::NUMERIC (19, 2)  AS net_mb
    , a.io_network_count::BIGINT                                                  AS net_cnt
    , ROUND( a.runtime_ms / 1000.0, 1 )::NUMERIC (9, 1)                           AS run_sec
    , (a.skew::NUMERIC(16,6) * 100)::NUMERIC (9, 2)                               AS skew_pct
    , CASE e.index
         WHEN 0 THEN ''/* query: ''      || q.query_id
               || '', database: ''       || q.database_name
               || '', type: ''           || q.type
               || '', cost: ''           || q.cost
               || e''\n** run secs: '' || ROUND( q.runtime_ms / 1000, 1 )
               || '',lock secs: ''       || ROUND( q.lock_ms / 1000, 1 )
...
-- ALTER FUNCTION log_query_steps_p( BIGINT )
