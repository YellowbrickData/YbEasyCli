      ...
      _sql := FORMAT( 'WITH 
       ...
       ,procs AS
         (  SELECT
             ...
             , CASE WHEN n.oid = 11 THEN t.typname 
					ELSE n.nspname||''.''||t.typname END              AS ret_type_name
         ...
         SELECT
          ...
          , REGEXP_REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( REPLACE( 
                                p.arg_type_names , ''character varying''          , ''VARCHAR''    )
                                                 ...
                                                 , ''(::[A-Z]+\S*)''              , '''', ''g''    )::VARCHAR(4000) 
                                                                                               AS arg_type_names
          , (CASE WHEN p.pronamespace = 11 THEN '''' WHEN p.proretset = ''t'' THEN ''SETOF '' ELSE '''' END
