...
BEGIN  
   ...
   */
   ...
   WITH db AS
      (  SELECT
         ...
         FROM
            sys.database
         WHERE
        	name ILIKE ' || quote_literal( _db_ilike ) || ' 
      )
    , db_schema AS 
      (  SELECT 
            s.database_id
          ...
          , s.name
         FROM sys.schema s 
         JOIN db         d on s.database_id = d.database_id
      )
    , table_storage AS
      (  SELECT
         ...
         FROM
            sys.table              t
            JOIN db                d  ON t.database_id = d.database_id
   ...
   SELECT
    ...
    , ts.tables                                                AS tables
    , ROUND (ts.rows         / 1000000.0, 2) ::numeric (18, 2) AS rows_mil
    , ROUND (ts.cmpr_bytes   / 1024.0^3, 2) ::numeric (18, 2)  AS cmpr_gb
    , ROUND (ts.uncmpr_bytes / 1024.0^3, 2) ::numeric (18, 2)  AS uncmpr_gb
    , ROUND (ds.cmpr_bytes   / 1024.0^3, 2) ::numeric (18, 2)  AS db_gb
    , ROUND ( DECODE(db_gb, 0, 0, cmpr_gb / db_gb) * 100.0, 1) ::numeric (12, 1)  AS pct_of_db
   FROM
      table_storage   ts
      JOIN db_storage ds ON ts.database_id = ds.database_id
      JOIN db_schema  s  ON ts.database_id = s.database_id AND ts.schema_id = s.schema_id     
      JOIN db         d  ON ts.database_id = d.database_id      
   WHERE 
        s.name ILIKE ' || quote_literal( _schema_ilike ) || '
    AND ' || _yb_util_filter || '
   ORDER BY 1, 2
   ';
