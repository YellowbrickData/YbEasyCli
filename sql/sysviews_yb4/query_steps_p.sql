...
BEGIN
   ...
   */
   ...
   _sql := 'SELECT
    ...
    , ROUND (qa.memory_planned_bytes/ 1024.0^2, 2) AS mem_plan_mb
    , ROUND (qa.memory_actual_bytes / 1024.0^2, 2) AS mem_actl_mb
    , ROUND (qa.io_read_bytes       / 1024.0^2, 2) AS io_read_mb
    , ROUND (qa.io_write_bytes      / 1024.0^2, 2) AS io_write_mb
    , ROUND (qa.io_network_bytes    / 1024.0^2, 2) AS io_net_mb
    , qa.io_network_count                          AS io_net_cnt
    , ROUND (qa.runtime_ms / 1000.0, 2)            AS run_sec
    , (qa.skew::NUMERIC(16,6) * 100)               AS skew_pct
    , e.query_plan                                 AS node_plan
   /*
    , e.type                                       AS step_type
    , e.index                                      AS index
