...
** TODO:
** . Add option for users or roles (groups)?
** . Need special handling for DFLTACL?
**
...
**  Example results:
**
**   db_name   | obj_type  | schema_name |  obj_name  | owner_name  |  grantee    |                       permissions ...
**-------------+-----------+-------------+------------+-------------+-------------+-----------------------------------------...
...
DROP TABLE IF EXISTS db_obj_user_perms_t CASCADE;

CREATE TABLE         db_obj_user_perms_t
(
   db_name      VARCHAR (128)
 , obj_type     VARCHAR (16)
...
*/
CREATE OR REPLACE PROCEDURE db_obj_user_perms_p(
     _user_ilike     VARCHAR
   ...
   RETURNS SETOF db_obj_user_perms_t
   LANGUAGE 'plpgsql'
   VOLATILE
...
DECLARE
   ...
   _user_rec            RECORD;
   _fn_name             VARCHAR(256) := 'db_obj_user_perms_p';
...
BEGIN
   ...
   */
   ...
   LOOP
      ...
      LEFT JOIN pg_catalog.pg_namespace   AS n  ON da.defaclnamespace = n.oid
      )
      ...
      /* There is no has_role_privilege, only pg_has_role(user, role, privilege): does user have privilege (MEMBER and/or USAGE) for role
      , roles AS
...
COMMENT ON FUNCTION db_obj_user_perms_p(VARCHAR, VARCHAR, VARCHAR, VARCHAR, INTEGER) IS 
$cmnt$Description:
...
Examples:
  SELECT * FROM db_obj_user_perms_p('dev_user1') 
  SELECT * FROM db_obj_user_perms_p('dev_user%') WHERE obj_type = 'SCHEMA'  
  SELECT * FROM db_obj_user_perms_p( 'dbadmin_r', '%', '%fact%' );
  SELECT * FROM db_obj_user_perms_p( 'dbadmin_r', 'public', _show_sql:=1 );
  
Arguments:
. _user_ilike     VARCHAR (reqd)   - An ILIKE pattern for the user name(s).
...
. If a user has no permissions on an object, the "permissions" column value
  will be NULL.
...
. This procedure is database specific; it does not work in a cross-database
  context.
. Role membership and grants are not included in this output.

Version:
. 2024.01.12 - Yellowbrick Technical Support
$cmnt$
;

\q
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~TEST~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
--GRANT EXECUTE ON db_obj_user_perms_p( VARCHAR, VARCHAR, VARCHAR, VARCHAR ) TO public;
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SELECT * FROM db_obj_user_perms_p('k%', _show_sql:=1                    ) LIMIT 40;
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SELECT * FROM db_obj_user_perms_p('y%', 'public'          );
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SELECT * FROM db_obj_user_perms_p('y%', 'p%', 'a%'        );
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SELECT * FROM db_obj_user_perms_p('%'                     ) WHERE obj_type = 'TABLE' AND permissions IS NOT NULL LIMIT 40;
\echo ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~DONE~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~