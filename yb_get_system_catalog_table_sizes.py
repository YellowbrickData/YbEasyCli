...
"""
import os, sys
from yb_common import Util, Report
...
class get_system_catalog_table_sizes(Util):
    units = 'BKMGTP'
    columns = OrderedDict([
        ('database_id'  , {'value': 'd.oid'                  , 'type': 'BIGINT'}),
        ('database_name', {'value': 'd.datname'              , 'type': 'VARCHAR(128)'}),
...
WHERE s.nspname IN ('sys', 'pg_catalog')
    AND c.relkind = 'r'
    AND CASE '{database}' WHEN 'yellowbrick' THEN TRUE ELSE NOT c.relisshared END
    AND total_size >= {total_size_min}
    AND {filter_clause};\n""".format(
          database = db
        , columns  = '\n,'.join(['{col} AS {dtype}'.format(col=v['value'], dtype=k) for k,v in self.columns.items()])
        , total_size_min = self.args_handler.args.total_size_min
