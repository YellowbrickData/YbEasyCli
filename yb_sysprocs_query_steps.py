...
"""
USAGE:
      yb_sysprocs_query_steps.py [options]
...
OPTIONS:
      ...
      (yb_sysprocs_query_steps.py --help)
...
class report_query_steps(SPReportUtil):
    """Issue the ybsql commands used to create the column distribution report."""
    config = {
        'description': 'Currently executing statements actual vs plan metrics by plan node.'
...
def main():
    print(report_query_steps().execute())
    exit(0)
