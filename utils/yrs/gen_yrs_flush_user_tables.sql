...
SELECT CASE row_number() OVER (ORDER BY :orderby)    WHEN 1 
   THEN '\c ' || NVL(d.name,'yellowbrick') || CHR(10) 
   ...
   || 'Flushing: ' || quote_ident(d.name) || '.' || quote_ident(s.name) || '.' || quote_ident(t.name) || CHR(10)
