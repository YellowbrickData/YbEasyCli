...
** Example Results:
**   \c kick;
**   \echo Thread 1: ................................Flushing user table: kick.public.ic
**   yflush public.ic;
**
** Revision History:
** . 2026.04.02 (rek) - Include PARTITION BY clause so set_gucs included with each db change
** . 2026.03.25 (rek) - Minor formatting updates
** . 2026.03.17 (rek) - Hide output generated by set_gucs.sql
** . 2026.03.11 (EM)  - Added JSONB GUC logic for 7.x
**                      Set JSONB guc and initial SQL only once per table.
...
\set num_groups 4
\set partitionby 'd.name'
\set orderby 'd.name, s.name, t.name'

SELECT CASE row_number() OVER (PARTITION BY :partitionby ORDER BY :orderby)    
   WHEN 1 THEN '\c ' || NVL(d.name,'yellowbrick') || CHR(10) 
   || '\o delete_' || (:grouping - 1)::VARCHAR || '.me' || CHR(10)
   || '\i set_gucs.sql' || CHR(10) 
   || '\timing on' || CHR(10) 
   || '\o' || CHR(10)
   ELSE '' 
   END
   || '\echo Thread ' || :grouping || ': ' || REPEAT('.',32) 
   || 'Flushing user table: ' || quote_ident(d.name) || '.' || quote_ident(s.name) || '.' || quote_ident(t.name) || CHR(10)
